...
chroms=[bamfile.get_reference_name(i) for i in range(len(bamfile.get_index_statistics())) ] 
...
temp_File = RESULT_PATH + PREFIX + ".tsd.temp*"
...
def merge_insertion(insertion1, insertion2):
    ...
    # insertion strand
    new_insertion_strand = '_'.join(list(set([insertion1[2], insertion2[2]])))
    
    new_insertion_type = insertion1[3] + ";" + insertion2[3]
    if insertion1[7] != insertion2[7]:
        new_insertion_type = insertion1[3] + ";" + insertion2[3] + ";" + "recombination"
    
    insertion_len = sum([insertion1[4], insertion2[4]])/2
    new_isnertion_seq = insertion1[5] + ";" + insertion2[5]
    ...
    te = '_'.join(list(set([insertion1[7], insertion2[7]])))
    ...
    return [new_insertion_start, new_insertion_end, new_insertion_strand,  new_insertion_type, insertion_len,  new_isnertion_seq, insertion_tag, te ]

# @profile
def record_insertion(ref, insertion, outfile_temp):
    # 将insertion记录到文件中
    # 这一步包括：
    #       · 判断insertion的方向 
    ...
    align_seq_file_name = RESULT_PATH + "/temp/" + PREFIX + "." + ref + ".seq.temp.fq"

    align_tsd_file_name = RESULT_PATH + "/temp/" + PREFIX + "." + ref + ".tsd.temp.fq"

    align_tsd_left_file_name = RESULT_PATH + "/temp/" + PREFIX + "." + ref + ".tsd.temp.1.fq"
    align_tsd_right_file_name = RESULT_PATH + "/temp/" + PREFIX + "." + ref + ".tsd.temp.2.fq"
    ...
    for read_seq in READS_SEQ_INFO.split(';'):           
        ...
        # print('>' + read_id + ".left")
        # print(insertion_sequence[5])
        # print('>' + read_id + ".right")
        # print(insertion_sequence[6])
    ...
    if insertion_end - insertion_start >= 50 or insertion_tag == 'truncked': 
        insertion_type = "GI|" + insertion_type
        tsd_seq = "NA"
    else:
        insertion_type = "NI|" + insertion_type
    ...
    outfile_temp.write('\t'.join(insertion_final) + '\n')
...
# @profile
def Cluster_reads_by_Position(chrom, TE_CLUSTER, bamfile):
# for ref in READS_CLUSTER:
    # TE_CLUSTER = READS_CLUSTER[ref]
    INSERTION_CLUSTER = []
    
    if len(TE_CLUSTER) == 0:
        return INSERTION_CLUSTER
    TE_CLUSTER =  sorted(TE_CLUSTER)
    insertion_pre = TE_CLUSTER[0]
    supporting_reads = 1
    # j = 0
    flag = '1'
    # cluster insertion by position
    for j in range(len(TE_CLUSTER[1:])):
        insertion = TE_CLUSTER[1:][j]
        # print(insertion)
        ...
        if insertion[0] >= insertion_pre[0] and insertion[0] <= insertion_pre[1]+20 and insertion[2] == insertion_pre[2] : # 判断方向
            ...
            mapped_reads = bamfile.count(contig=chrom, start=insertion_pre[0], stop=insertion_pre[1]+500)
            # insertion_start, insertion_end, insertion_type, clip_te_len, cilp_seq, insertion_tag
            insertion_record = [insertion_pre[0], insertion_pre[1], supporting_reads, mapped_reads,  insertion_pre[3], insertion_pre[4], insertion_pre[5], insertion_pre[6], insertion_pre[7]  ]
            INSERTION_CLUSTER.append([chrom, insertion_record])
            # record_insertion(ref,insertion_record)
            insertion_pre = insertion
            supporting_reads = 1
            if j == len(TE_CLUSTER[1:]):
                flag='end'
...
# INSERTION_RES = []
# @profile
def process_insertion(chrom):
    bamfile = pysam.AlignmentFile( DATA_PATH + BAMFILE, "rb" )
    outfile_temp = open(RESULT_PATH + "/temp/" + PREFIX + "." + chrom + ".lrft.table.txt", 'w')

    READS = bamfile.fetch(chrom)
    # region = "21:44,456,296-44,456,395"
    # chr = region.split(':')[0]
    # region_start = region.split(':')[1].split('-')[0].replace(',', '')
    # region_end = region.split(':')[1].split('-')[1].replace(',', '')
    # READS = bamfile.fetch( chr, int(region_start) , int(region_end))

    TE_CLUSTER = Cluster_reads_by_TE(READS, READS_SEQUENCE)
    INSERTION_CLUSTER = Cluster_reads_by_Position(chrom, TE_CLUSTER, bamfile)
    for record in INSERTION_CLUSTER:
        record_insertion(record[0], record[1], outfile_temp)
    outfile_temp.close()
    return INSERTION_CLUSTER

# @profile
def main():
    pool = multipro.Pool(processes=int(process_num))
    pool.map(process_insertion, chroms)
    pool.close()
    pool.join()
    merge_script = "cat " + RESULT_PATH + "/temp/" + PREFIX + "*.txt" + " >> " + outFile 
...
# rm_args = "rm " + RESULT_PATH + "temp/*"
# subprocess.Popen(rm_args, shell=True)
# rm_args = ['rm', temp_File]
